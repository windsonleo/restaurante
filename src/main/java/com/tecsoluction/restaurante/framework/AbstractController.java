...
public abstract class AbstractController<Entity> {
	...
	public ModelAndView editarEntity(@ModelAttribute @Valid Entity entity, BindingResult result,
		...
		if (result.hasErrors()) {
			...
			System.out.println(
					"erro ao Editar Entidade: " + entityAlias + " outros erros global: " + result.getGlobalError());
			System.out.println(
					"erro ao Editar Entidade: " + entityAlias + " outros erros nestedPatch: " + result.getNestedPath());
			attributes.addFlashAttribute("erros", "Erro ao Salvar." + result.getFieldError());
